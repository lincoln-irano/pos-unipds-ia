...
const config = {
	...
	DATABASE_URL: env
		.get("DATABASE_URL")
		.default(
			`postgresql://alumnus:alumnus_dev_password@localhost:5433/${defaultAppName}`,
		)
		.required()
		.asString(),
	...
	OTEL_EXPORTER_OTLP_ENDPOINT: env
		...
		.required()
		.asString(),
	OTEL_EXPORTER_OTLP_COMPRESSION: env
		.get("OTEL_EXPORTER_OTLP_COMPRESSION")
		.default("gzip")
		.required()
		.asString(),
