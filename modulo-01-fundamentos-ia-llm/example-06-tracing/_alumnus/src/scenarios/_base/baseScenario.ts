import type { AppInstance, Config, DatabaseConnection } from "../../types";
...
export abstract class BaseScenario {
	...
	async registerRoutes(
		...
		if (!this.#initialized) {
			throw new Error(
				`Scenario ${this.name} must be initialized before registering routes`,
			);
		}
	...
	async terminate(app: AppInstance): Promise<void> {
		if (!this.#initialized) {
			app.log.warn(
				`Scenario ${this.name} not initialized, skipping termination`,
			);
			return;
		}
	...
	protected async _registerRoutes(
		_app: AppInstance,
		_config: Config,
		_db: DatabaseConnection,
	): Promise<void> {
