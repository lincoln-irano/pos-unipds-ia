...
export const config = {
	...
	waitForScrapeInterval: envVar
		.get("WAIT_FOR_SCRAPE_INTERVAL")
		.default(5000)
		.required()
		.asIntPositive(),
};
...
export class alumnusAPI {
	...
	async checkServices(): Promise<void> {
		...
		const mainAppReady = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.health}`,
		);
		...
		const promHealth = await util.fetchWithRetry(
			config.prometheus.getHealthyUrl(),
		);
	...
	async getStudents(): Promise<void> {
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.students}`,
		);
	...
	async getStudentsLoadBalancer(): Promise<void> {
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsLoadBalancer}`,
		);
	...
	async getStudentsRedisCache(): Promise<void> {
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsRedisCache}`,
		);
	...
	async getStudentsWorkerThreads(): Promise<void> {
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsWorkerThreads}`,
		);
	...
	async flushRedisCache(): Promise<void> {
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsRedisCache}/flush`,
			{
				method: "DELETE",
			},
		);
	...
	async getStudentsDbLeakyConnections(): Promise<{
		...
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsDbLeakyConnections}`,
		);
	...
	async getDbLeakyConnectionsHealth(): Promise<{
		...
		const response = await util.fetchWithRetry(
			`${this.defaultAppEndpoint}${config.app.endpoints.studentsDbLeakyConnectionsHealth}`,
		);
		assert.ok(
			response?.ok,
			"DB leaky connections health request should succeed",
		);
		return (await response.json()) as Promise<{
			leakedConnections: number;
			status: string;
