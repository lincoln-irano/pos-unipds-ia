...
describe("DB Leaky Connections Scenario - Connection Leak Detection", () => {
	...
	it("should verify logs are registered in Loki with connection leak context", async () =>
		await util.retry(async () => {
			...
			assert.strictEqual(
				lokiResponse.status,
				"success",
				"Loki query should succeed",
			);
			assert.ok(lokiResponse.data, "Loki response should contain data");
			assert.ok(
				lokiResponse.data.result,
				"Loki response should contain result array",
			);
			...
			assert.strictEqual(
				hasLogs,
				true,
				`Loki should contain logs from ${serviceName}`,
			);
			...
			for (const logStream of lokiResponse.data.result) {
				...
				const logEntries =
					logStream.values.map((v: [string, string]) => v[1]) || [];
				const found = logEntries.some((log: string) => {
					try {
						// Try parsing as JSON (structured logs)
						const parsed = JSON.parse(log);
						return (
							parsed.msg?.includes("Query executed successfully") ||
						...
						return (
							log.includes("Query executed successfully") ||
			...
			assert.ok(
				hasConnectionLogs,
				"Logs should contain connection-related entries",
			);
		}));
	...
	it("should verify traces are captured in Tempo", async () => {
		...
		return await util.retry(
			async () => {
				...
				const tempoResponse: TempoResponse = await util.queryTempo(
					url,
					serviceName,
				);
				...
				assert.ok(
					tempoResponse.traces.length > 0,
					`Tempo should contain traces from ${serviceName}`,
				);
				...
				assert.strictEqual(
					...
					serviceName,
					`Trace should be from ${serviceName}`,
				);
			},
			10,
			3000,
	...
	it("should verify metrics are registered in Prometheus", async () =>
		await util.retry(async () => {
			const url = config.prometheus.getQueryUrl();
			const promResponse: PrometheusResponse = await util.queryPrometheus(
				url,
				serviceName,
			);
			...
			assert.ok(
				promResponse.data.result,
				"Prometheus response should contain result array",
			);
			...
			assert.strictEqual(
				hasMetrics,
				true,
				`Prometheus should contain metrics from ${serviceName}`,
			);
			...
			assert.strictEqual(
				metric.exported_job,
				serviceName,
				`Metric should be from ${serviceName} service`,
			);
	...
	it.skip("should verify traces show requests to leaky endpoint", async () =>
		await util.retry(
			async () => {
				const url = config.tempo.getSearchUrl();
				const tempoResponse: TempoResponse = await util.queryTempo(
				...
				for (const trace of tempoResponse.traces) {
					...
					if (
						trace.rootTraceName &&
						trace.rootTraceName.includes("/students/db-leaky-connections")
					) {
					...
					if (trace.spanSets) {
						for (const spanSet of trace.spanSets) {
							const hasRoute = spanSet.spans.some((span) => {
								...
								if (
									span.name &&
									span.name.includes("/students/db-leaky-connections")
								) {
									return true;
								}
				...
				assert.ok(
					hasLeakyEndpointTrace,
					"Should have traces for /students/db-leaky-connections endpoint",
				);
			},
			5,
			2000,
